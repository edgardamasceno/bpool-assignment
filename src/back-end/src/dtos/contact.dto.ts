...
import { IsString, IsNotEmpty, IsEmail, IsEnum, IsDate, MinLength, MaxLength, MinDate, MaxDate } from 'class-validator';
...
export class CreateContactDto {
  ...
  @MinLength(3, {message: 'O campo E-MAIL deve conter no mínimo 5 caracteres.'})
  @MaxLength(50, {message: 'O campo E-MAIL deve conter no máximo 100 caracteres.'})
  ...
  @MinDate(new Date('01/01/1850'), {message: 'O campo DATA deve ser superior a 01/01/1850.'})
  @MaxDate(new Date(), {message: 'O campo DATA deve ser igual ou inferior à data atual.'})
  readonly birth: Date;

  @IsEnum(Occupation, {message: 'O campo PROFISSÃO deve conter uma das opçoes: Médico, Professor ou Analista Sistemas.'})
